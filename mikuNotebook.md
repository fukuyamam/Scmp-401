...
### Manual on how to use the workflow
If you want to enter new data into the workflow, first delete all of the current data that is in the workflow, which is from January 2018 to mid February 2018. Then, after checking that all of the data has been deleted, export the data that you would like to analyze from LGL in Excel form. Once the Excel sheet is downloaded, highlight and copy the entire spreadsheet, and paste it into the workflow. Now, on the bottom of the workflow, there are three tabs: one called Monthly/YearlyMembers, one called NumChildrenByAge, and one called TransactionsPerHour. In the Monthly/YearlyMembers tab, you will see the number of monthly members and the number of yearly members that have come into SPI during the time frame you are studying. In the NumChildrenByAge tab, you will see the number of children within each age category (from 1 to 12+) that have come in during this time. In the TransactionsPerHour tab, you will be able to see how many transactions per hour occurred in each of your business hours. This last tab could be especially helpful in figuring out when more people should be working. The NumChildrenByAge tab could also be helpful in seeing which demographic you should most be catering to.
